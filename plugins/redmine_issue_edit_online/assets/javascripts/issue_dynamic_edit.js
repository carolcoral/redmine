 ...
 * See https://github.com/carolcoral/redmine_issue_edit_online/commit/26684a2dd9b12dcc7377afd79e9fe5c142d26ebd for more info
 */
...
const getEditFormHTML = function(attribute){
	...
	// Special handling for description field to ensure we find it in Redmine 6.x
	if(attribute === "description") {
		formElement = document.querySelector('#issue_description');
		if(!formElement) {
			formElement = document.querySelector('textarea[name="issue[description]"]');
		}
	}
	
	// General case for other fields
	if(!formElement) {
		formElement = document.querySelector('#issue_' + attribute + "_id");
...
const cloneEditForm = function(){
	// Fix: Remove existing edit elements to prevent duplicates
	const existingIcons = document.querySelectorAll('.issue.details .iconEdit, .issue.details .dynamicEditField, .issue.details .refreshData');
	...
	const subjectDiv = document.querySelector('.issue.details div.subject');
	if (subjectDiv) {
		subjectDiv.insertBefore(btn_refresh, null);
	}
	...
	document.querySelectorAll('div.issue.details .attribute').forEach(function(elt){
		...
		if(attributes && !_CONF_EXCLUDED_FIELD_ID.includes(selected_elt)){
			...
			if(dynamicEditField){
				...
				// Fix: Add null check for .value element to prevent JavaScript errors
				const valueElement = elt.querySelector('.value');
				if (valueElement) {
	...
	if(!_CONF_EXCLUDED_FIELD_ID.includes("description")){
		let descriptionContainer = document.querySelector('div.issue.details .description');
		const issueDetails = document.querySelector('div.issue.details');
		
		// If description container doesn't exist, create it
		if(!descriptionContainer && issueDetails) {
			console.log('[redmine_issue_edit_online] Creating description container');
			...
			// Insert after attributes (as sibling of attributes)
			const attributes = issueDetails.querySelector('div.attributes');
			if(attributes && attributes.parentNode) {
				// Insert after attributes
				attributes.parentNode.insertBefore(descriptionContainer, attributes.nextSibling);
				console.log('[redmine_issue_edit_online] Description container inserted after attributes');
			} else {
				// Fallback: insert at the end of issue details
				issueDetails.appendChild(descriptionContainer);
				console.log('[redmine_issue_edit_online] Description container appended to issue details');
			}
		}
		
		if(descriptionContainer) {
			const descriptionHeader = descriptionContainer.querySelector('p strong');
			if(descriptionHeader) {
				const btn_edit = document.createElement('span');
				btn_edit.classList.add('iconEdit');
				btn_edit.innerHTML = SVG_EDIT;
				descriptionHeader.insertAdjacentElement("afterend", btn_edit);
			}
			
			const formDescription = getEditFormHTML("description");
			if(formDescription) {
				
				const descriptionInput = formDescription.querySelector("#issue_description_dynamic");
				if(descriptionInput) {
					descriptionInput.removeAttribute('data-tribute');
				}
				descriptionContainer.insertBefore(formDescription, null);

				// Initialize toolbar after the element is inserted into DOM
				setTimeout(() => {
					if (typeof(CKEDITOR) !== "undefined" && CKEDITOR.instances['issue_description']) {
						const cfg = CKEDITOR.instances['issue_description'].config;
						cfg.height = 100;
						if (CKEDITOR.instances['issue_description_dynamic']) {
							CKEDITOR.instances['issue_description_dynamic'].destroy(true);
						}
						CKEDITOR.replace("issue_description_dynamic", cfg);
					} else if (typeof(jsToolBar) !== "undefined" && document.querySelector('#issue_description_dynamic')) {
...
document.querySelector('body').addEventListener('click', function(e){
	if(e.target.matches('.dynamicEditField .action.valid') || e.target.closest('.dynamicEditField .action.valid')){
		e.preventDefault();
		let container = e.target.closest('.dynamicEditField');
		let inputs = container.querySelectorAll('*[name]');
		let formData = [];
		let existingIndex = [];
		...
		inputs.forEach(elt => {
			...
			if(elt.matches('input[type="radio"]:checked') || elt.matches('input[type="checkbox"]:checked') || not_multiple){
				if(!existingIndex.includes(elt.getAttribute('name'))){
					...
					if(elt.id === 'issue_description_dynamic' && typeof(CKEDITOR) !== 'undefined' && CKEDITOR.instances['issue_description_dynamic']) {
						value = CKEDITOR.instances['issue_description_dynamic'].getData();
					}
					
					formData.push({"name" : elt.getAttribute('name'), "value" : value});
				}
			}
		});
		...
		request.onreadystatechange = function() {
			if (this.readyState == 4) {
				if(this.status == 200) {
					...
					// Fix: Preserve description content when updating attributes
					const currentDescription = document.querySelector('div.issue.details .description');
					const newDetails = doc.querySelector('div.issue.details');
					
					if(currentDescription && newDetails) {
						const newDescription = newDetails.querySelector('.description');
						if(!newDescription) {
							// If new HTML doesn't contain description, preserve the current one
							const tempDiv = document.createElement('div');
							tempDiv.innerHTML = newDetails.innerHTML;
							tempDiv.appendChild(currentDescription.cloneNode(true));
							document.querySelector('div.issue.details').innerHTML = tempDiv.innerHTML;
						} else {
							document.querySelector('div.issue.details').innerHTML = newDetails.innerHTML;
						}
					} else if(document.querySelector('div.issue.details')) {
						document.querySelector('div.issue.details').innerHTML = doc.querySelector('div.issue.details').innerHTML;
					}
					
					// Fix: Preserve tab-notes content when updating attributes
					const currentTabNotes = document.querySelector('#tab-notes');
					if(currentTabNotes) {
						const newTabNotes = doc.querySelector('#tab-notes');
						if(!newTabNotes && document.querySelector('div.issue.details')) {
							// If new HTML doesn't contain tab-notes, preserve the current one
							const detailsContainer = document.querySelector('div.issue.details');
							if(detailsContainer.parentNode) {
								detailsContainer.parentNode.insertBefore(currentTabNotes.cloneNode(true), detailsContainer.nextSibling);
							}
						}
					}
					...
					// Update history - Redmine 6.1.1 compatibility: check for different history container selectors
					let historyContainer = document.querySelector('#history');
					if (!historyContainer) {
						// Try alternative selectors for Redmine 6.x
						historyContainer = document.querySelector('#issue-changesets');
					}
					
					if (historyContainer) {
						const newHistoryContent = doc.querySelector('#history') || doc.querySelector('#issue-changesets');
							if (newHistoryContent && newHistoryContent.innerHTML.trim()) {
								// Replace all history content
...
try {
	...
	console.error('[redmine_issue_edit_online] Plugin initialization failed:', error);
}
