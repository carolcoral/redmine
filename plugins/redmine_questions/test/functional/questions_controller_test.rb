...
class QuestionsControllerTest < ActionController::TestCase
  ...
  def test_get_index_with_pagination
    # create 30 additional questions
    30.times do |question_number|
      question = Question.new(subject: "question_#{question_number + 5}", content: 'test text', author_id: 1, section_id: 1)
      question.save
    end
    ...
    # first page request
    compatible_request :get, :index, :section_id => section.id, :project_id => @project
    assert_response :success
    assert_equal 32, @project.questions.count
    assert_select 'div.topic h3.subject', {:count => 25}

    # second page request
    compatible_request :get, :index, :section_id => section.id, :project_id => @project, page: 2
    assert_select 'div.topic h3.subject', {:count => 7}
  end
