module RedmineYjs
  class Hooks < Redmine::Hook::ViewListener
    ...
    def view_layouts_base_html_head(context = {})
      Rails.logger.debug "[Yjs Hook] view_layouts_base_html_head called"
      ...
      result << view.javascript_tag("if(window.RedmineYjsConfig&&window.RedmineYjsConfig.debug)console.log('[Yjs] Hook fired!');").html_safe
      ...
      if view.respond_to?(:yjs_enabled?)
        ...
        Rails.logger.debug "[Yjs Hook] Yjs enabled: #{enabled_value}"
      elsif settings
        # Fallback: check settings directly (helper might not be available in hook context)
        enabled_value = settings['yjs_enabled'] == '1'
        Rails.logger.debug "[Yjs Hook] Yjs enabled: #{enabled_value} (via settings)"
      else
        Rails.logger.warn "[Yjs Hook] No settings found, plugin disabled"
      end
      
      if enabled_value
        Rails.logger.debug "[Yjs Hook] Yjs enabled, rendering assets"
          ...
          url = settings && settings['hocuspocus_url'] || 'ws://localhost:3000/ws'
            ...
            Rails.logger.debug "[Yjs Hook] Hocuspocus URL: #{hocuspocus_url}"
        ...
        if document_context.any?
          Rails.logger.debug "[Yjs Hook] Document context: #{document_context.inspect}"
        else
        ...
        if File.exist?(bundled_deps_plugin) || File.exist?(bundled_deps_public)
          Rails.logger.debug "[Yjs Hook] Using bundled dependencies"
          ...
          Rails.logger.debug "[Yjs Hook] Using CDN dependencies"
        ...
        Rails.logger.debug "[Yjs Hook] Yjs disabled"
      end
